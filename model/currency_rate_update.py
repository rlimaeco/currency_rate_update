...
BANREP_supported_currency_array = [
    "ARS", "ATS", "AUD", "BEF", "BRL", "CAD", "CHF", "CLP", "CNH", "CNY",
    "COP", "DEM", "DKK", "ECS", "ESP", "EUR", "FRF", "GBP", "GTQ", "HKD",
    "IDR", "INR", "ITL", "JPY", "KRW", "MXN", "NLG", "NOK", "NZD", "PAB",
    "RUB", "SEK", "SGD", "TRY", "UYU", "VEF-DICOM", "VEF-DIPRO", "USD"]
...
#INFO
# Para pesos colombianos (COP) utilizar o getter
# >>> CA_BOC_getter


supported_currecies = {
    'BANREP_getter': BANREP_supported_currency_array,
...
class Currency_rate_update_service(models.Model):
    ...
    _description = _("Currency Rate Update")
    ...
    def _onchange_interval_number(self):
        if self.interval_number == 0:
            self.note = _('%s Service deactivated. Currencies will no longer ' \
    ...
    service = fields.Selection(
        [('CH_ADMIN_getter', 'Admin.ch'),
         ...
         ('BANREP_getter', _('Bank of the Republic of Colombia')),
         # Added for polish rates
         ('PL_NBP_getter', _('National Bank of Poland')),
         # Added for mexican rates
         ('MX_BdM_getter', _('Bank of Mexico')),
         ...
         ('CA_BOC_getter', _('Bank of Canada - noon rates')),
         # Added for romanian rates
         ('RO_BNR_getter', _('National Bank of Romania'))
         ],
        string=_("Webservice to use"),
        required=True)
    ...
    note = fields.Text(_('Update logs'))
    max_delta_days = fields.Integer(
        string=_('Max delta days'), default=4, required=True,
        help=_("If the time delta between the rate date given by the "
    ...
    interval_type = fields.Selection([
        ...
        ('weeks', _('Week(s)')),
        ('months', _('Month(s)'))],
        string=_('Currency update frequency'),
        default='days')
    interval_number = fields.Integer(string=_('Frequency'), default=1)
    next_run = fields.Date(string=_('Next run on'), default=fields.Date.today())
    ...
    def refresh_currency(self):
        ...
        _logger.info(
            _('Starting to refresh currencies with service %s (company: %s)'),
            ...
            try:
                ...
                for curr in self.currency_to_update:
                    ...
                    if do_create:
                        ...
                        _logger.info(
                            _('Updated currency %s via service %s'),
    ...
    def _run_currency_update(self):
        _logger.info(_('Starting the currency rate update cron'))
        self.run_currency_update()
        _logger.info(_('End of the currency rate update cron'))
